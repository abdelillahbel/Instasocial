...
public class ScaleImageView extends android.support.v7.widget.AppCompatImageView {
	...
	private boolean scaleToWidth = false; // this flag determines if should measure viewHeight manually dependent of viewWidth
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		/**
		 * if both viewWidth and viewHeight are set scale viewWidth first. modify in future if necessary
		 */
		...
		if(widthMode == MeasureSpec.EXACTLY || widthMode == MeasureSpec.AT_MOST){
			...
			scaleToWidth = false;
		}else throw new IllegalStateException("viewWidth or viewHeight needs to be set to match_parent or a specific dimension");
		...
		if(getDrawable()==null || getDrawable().getIntrinsicWidth()==0 ){
			...
			if(scaleToWidth){
				...
				int heightC = viewWidth *ih/iw;
				if(viewHeight > 0)
				if(heightC> viewHeight){
					// dont let hegiht be greater then set max
					heightC = viewHeight;
					viewWidth = heightC*iw/ih;
				}
				
				this.setScaleType(ScaleType.FIT_XY);
				setMeasuredDimension(viewWidth, heightC);
				
			}else{
				// need to scale to viewHeight instead
				int marg = 0;
				if(getParent()!=null){
