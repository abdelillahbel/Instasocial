...
//
    ...
    private static GPUImageFilter createBlendFilter(Context context, Class<? extends GPUImageTwoInputFilter> filterClass) {
        try {
            ...
            filter.setBitmap(BitmapFactory.decodeResource(context.getResources(), R.drawable.about_logo));
            return filter;
        } catch (Exception e) {
            e.printStackTrace();
            return null;
    ...
    public static class FilterAdjuster {
        ...
        public FilterAdjuster(final GPUImageFilter filter) {
            ...
            if (filter instanceof GPUImageSharpenFilter) {
                ...
                Log.e("filter", "filter instanceof GPUImageSepiaFilter");
            ...
            } else if (filter instanceof GPUImageHiglightFilter) {
                adjuster = new HighlightAdjuster().filter(filter);
            } else if (filter instanceof GPUImageColorMatrixFilter) {
                adjuster = new ColorMatrixAdjuster().filter(filter);
            ...
            } else if (filter instanceof GPUImageDarkShadow) {
                adjuster = new StructureAdjuster().filter(filter);
            } else if (filter instanceof GPUImageGaussianBlurFilter) {
                adjuster = new GaussianAdjuster().filter(filter);
            } else {
                adjuster = null;
            }
        }
        ...
        private class BrightnessAdjuster extends Adjuster<GPUImageBrightnessFilter> {
            @Override
            public void adjust(final int percentage) {
                getFilter().setBrightness(range(percentage, 0f, 0.80f));
            }
        }
        ...
        private class StructureAdjuster extends Adjuster<GPUImageDarkShadow> {
            @Override
            public void adjust(final int percentage) {
        ...
        private class GaussianAdjuster extends Adjuster<GPUImageGaussianBlurFilter> {
            @Override
            public void adjust(final int percentage) {
                getFilter().setBlurSize(2f);
                getFilter().getVerticalTexelOffsetRatio();
            }
        }
        ...
        private class HighlightAdjuster extends Adjuster<GPUImageHiglightFilter> {
            @Override
            public void adjust(final int percentage) {
                getFilter().setShadows(range(percentage, 0.0f, 1.0f));
                getFilter().setHighlights(range(percentage, 0.0f, 1.0f));
        ...
        private class ColorMatrixAdjuster extends Adjuster<GPUImageColorMatrixFilter> {
            @Override
            public void adjust(final int percentage) {
        ...
        private class WhiteBalanceAdjuster extends Adjuster<GPUImageWhiteBalanceFilter> {
            @Override
            public void adjust(final int percentage) {
                getFilter().setTemperature(range(percentage, 4000.0f, 7000.0f));
        ...
        private class VignetteAdjuster extends Adjuster<GPUImageVignetteFilter> {
            @Override
            public void adjust(final int percentage) {
                getFilter().setVignetteStart(range(percentage, 0.5f, 0.0f));
            }
        }
